...
void update_fdata(Fdata *f, char *str) {
    ...
    if((eq_ptr = strchr(str, '=')) == NULL) {
		fprintf(stderr, "Error: badly formatted query string\n");
		exit(1);
	} 
	*eq_ptr = '\0';
	eq_ptr++;
...
/* Return a array of name value pairs given a query string.
...
Fdata *parse_query(char *str) {
	...
	// Make the fields of last element of f point to NULLs so that 
	// we have a sentinel. (A sentinel is a special value that tells
	// us we have reached the end of the list.)
	f[i].name = NULL;
	f[i].value = NULL;
	return f;
}
