...
void initClients(struct clientstate *clients, int size) {
    // Initialize client array
    for (int i = 0; i < size; i++){
        clients[i].sock = -1;	/* -1 indicates available entry */
        clients[i].fd[0] = -1;
        clients[i].request = NULL;	
        clients[i].path = NULL;
        clients[i].query_string = NULL;
        clients[i].output = NULL;
        clients[i].optr = clients[i].output;
    }
}
